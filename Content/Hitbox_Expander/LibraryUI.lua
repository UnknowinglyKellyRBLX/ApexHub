	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateSlider(SliderSettings)
        	...
        	local function UpdateProgress(Value)
        		...
        		local NewSize = math.max(Slider.Main.AbsoluteSize.X * Percent, 5)
        		Slider.Main.Progress.Size = UDim2.new(0, NewSize, 1, 0)
        		...
        		if SliderSettings.CurrentValue ~= Value then
        			local Success, Err = pcall(SliderSettings.Callback, Value)
        			if not Success then
        				TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = Color3.fromRGB(85,0,0)}):Play()
        				TweenService:Create(Slider.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
        				Slider.Title.Text = "Callback Error"
        				warn("Rayfield | "..SliderSettings.Name.." Callback Error: "..tostring(Err))
        				task.wait(0.5)
        				Slider.Title.Text = SliderSettings.Name
        	...
        	local function GetValueFromX(X)
        		local RelativeX = math.clamp(X - Slider.Main.AbsolutePosition.X, 0, Slider.Main.AbsoluteSize.X)
        		local RawValue = SliderSettings.Range[1] + (RelativeX / Slider.Main.AbsoluteSize.X) * (SliderSettings.Range[2] - SliderSettings.Range[1])
        		return math.floor(RawValue / SliderSettings.Increment + 0.5) * SliderSettings.Increment
        	end
        
        	UpdateProgress(SliderSettings.CurrentValue)
        	...
        	local function StartDrag()
        		SLDragging = true
        		TweenService:Create(Slider.Main.UIStroke, TweenInfo.new(0.2), {Transparency = 1}):Play()
        		TweenService:Create(Slider.Main.Progress.UIStroke, TweenInfo.new(0.2), {Transparency = 1}):Play()
        	end
        
        	local function EndDrag()
        		SLDragging = false
        		TweenService:Create(Slider.Main.UIStroke, TweenInfo.new(0.2), {Transparency = 0.4}):Play()
        		TweenService:Create(Slider.Main.Progress.UIStroke, TweenInfo.new(0.2), {Transparency = 0.3}):Play()
        	end
